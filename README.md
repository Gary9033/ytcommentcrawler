# YouTube Comment Crawler (ytcommentcrawler) 🚀
> A simple way to save your time ~

這是一個用 Python 寫的 YouTube 留言爬蟲工具，可以抓取影片留言並生成文字雲。
## 🛠 功能特色
* 自動抓取指定影片的所有留言。
* 支援文字雲（Word Cloud）生成。
* 自動過濾無效字元。

## 📦 安裝需求
請確保你的電腦已安裝 Python 3.x，並執行以下指令安裝必要套件：

```bash
pip install pandas wordcloud matplotlib requests
🚀 使用方法
下載 cloud.py。

在終端機執行：

Bash
python cloud.py
依照提示輸入 YouTube 影片網址。

📂 檔案結構
cloud.py: 主程式邏輯。

.gitignore: 忽略暫存檔與環境設定。

README.md: 專案說明文件。

📜 授權條款
本專案採用 MIT License 授權。


---

### 接下來的操作
既然內容都準備好了，現在就來實戰最後一次 **Git 三部曲**：

1. **存檔**：在你的 `ytcommentcrawler` 資料夾裡新增一個 `README.md`，把上面的內容貼進去存檔。
2. **執行指令**：
   ```powershell
   git add README.md
   git commit -m "docs: 完成美化版 README"
   git push```